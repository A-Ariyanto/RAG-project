...
- Frontend: deliberately minimal single-page chat view — the backend is the star. **Vite + React + Tailwind**, in a `frontend/` directory in this repo (not a separate repo: one deploy story, no CORS friction when served same-origin, and the README keeps telling one story). Minimal today, but the tooling won't need swapping if the UI grows later.
...
Vite + React + Tailwind app in `frontend/`: single-page chat view consuming the SSE stream, rendering citations as links to source URLs. Serve the built bundle from FastAPI (same origin — no CORS config, relative `/ask` URL works locally and deployed). Timebox hard — the backend is the star.
